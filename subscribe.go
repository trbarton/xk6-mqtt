...
func (c *client) receiveMessageMetric(msgLen float64) error {
	...
	metrics.PushIfNotDone(ctx, state.Samples, metrics.Sample{
		TimeSeries: metrics.TimeSeries{Metric: c.metrics.ReceivedMessages, Tags: c.metrics.TagsAndMeta.Tags.WithTagsFromMap(additionalTags)},
		Time:       now,
		Value:      float64(1),
	})
	metrics.PushIfNotDone(ctx, state.Samples, metrics.Sample{
		TimeSeries: metrics.TimeSeries{Metric: c.metrics.ReceivedBytes, Tags: c.metrics.TagsAndMeta.Tags.WithTagsFromMap(additionalTags)},
		Time:       now,
		Value:      msgLen,
	})
	return nil
